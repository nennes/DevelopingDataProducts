...
amort <- function(principal, interest, duration, payfreq, firstpay, compoundfreq) {
    ...
    data$payment <- 0
    data$payment[ (data$month - firstpay) >= 0 & (data$month - firstpay) %% payfreq == 0 ] <- round(pay$payment,2)
    i <- which(data$payment != 0)
    i <- i[length(i)]
    data$payment[ i ] <- 0
    data$payment[ i ] <- pay$payment * (duration - (firstpay - 1) / 12) * 12 / payfreq - sum(data$payment)
    data$totalPaid <- round(cumsum(data$payment),2)
    ...
    data$principal[ idx.pr ] <- round((1 + pay$r)^seq_len(length(idx.pr)) * pay$principal - ( (1 + pay$r)^seq_len(length(idx.pr)) - 1 ) / pay$r * pay$payment * compoundfreq / payfreq,2)
...
shinyServer(function(input, output, session) {
    ...
    output$payment  <- renderPrint({
        ...
        cat("The recurrent payment (every ")
        if(input$payfreq > 1) {
            cat(input$payfreq)
            cat(" months")
        } else {
